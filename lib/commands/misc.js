...
export const help = {
  ...
  usage: '!help',
  execute (client, message) {
    message.channel.send(map(filter(commands, c => typeof c.usage !== 'undefined'), command => {
...
export const test = {
  name: 'Test',
  help: 'Command to test arguements',
  usage: '!test',
  execute (client, message) {
    try {
      messageArguments(message, { 'competitive': 'flag', 'style': 'map', 'positional': 1 })
    } catch (error) {
