...
export function chooseGame (client, message, options) {
  const users = usersInVoiceChannel(client, message)
  if (users.length === 0) { return undefined }
  const userCount = users.length
  const games = config.get('games')
  return sample(filterGames(games, userCount, options))
}
...
function matchesTheOptions (game, options) {
  for (const option in options) {
    if (option in game) {
      if (String(game[option]) !== options[option]) { return false }
    }
  }
  return true
}
